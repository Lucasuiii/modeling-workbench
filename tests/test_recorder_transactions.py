...
class RecorderTransactions(unittest.TestCase):
    ...
    def test_backend_note_does_not_break_actual_recording(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            m = json.loads((p/'runs/NOTE/RUN_MANIFEST.json').read_text())
        ...
        for reason in ('source_changed', 'missing_capability', 'invalid_assertions'):
            for preexisting in (True, False):
                with self.subTest(reason=reason, preexisting=preexisting), tempfile.TemporaryDirectory() as d:
                    ...
                    for rel in paths:
                        if preexisting:
                            (p/rel).write_text('old evidence')
                    code = 'from pathlib import Path\n'
                    code += 'Path("results/claim.json").write_text("new evidence")\n'
                    code += 'Path("results/verdict.json").write_text('+repr('invalid json' if reason == 'invalid_assertions' else '[]')+')\n'
                    if reason == 'source_changed':
                        code += 'Path(__file__).write_text("# changed source")\n'
                    (p/'code/transaction.py').write_text(code)
                    ...
                    for rel in paths:
                        if preexisting:
                            self.assertEqual((p/rel).read_text(), 'old evidence')
                        else:
    ...
    def test_manifest_install_failure_rolls_back(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(json.loads((p/'runs/WRITEFAIL/rejected_outputs/results/q1_output.json').read_text())['minimum_cost'], 1.5)

    def test_committed_manifest_keeps_new_live_output(self):
        with tempfile.TemporaryDirectory() as d:
            p = make_project(Path(d))
            (p/'results/q1_output.json').write_text('old evidence')
            ...
            self.assertEqual(json.loads((p/'results/q1_output.json').read_text())['minimum_cost'], 1.5)
