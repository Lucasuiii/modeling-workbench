...
| 场景 | 可复用的指导 |
|---|---|
| 华为杯、MathorCup、电工杯及区域建模赛等固定题比赛 | 题意拆解、候选模型、计算证据、机制验证与论文论证 |
| MCM/ICM、APMCM 等英文任务 | 同一建模与复核思路，补充英文摘要、术语和按题目要求撰写的文书 |
| 泰迪杯及其他数据分析任务 | 数据口径、清洗、泄漏防范、基线与任务匹配的评价 |
| 统计建模及自主选题任务 | 主题约束、数据可得性、研究设计、团队研究问题与可支持的结论 |

这些名称是使用入口，不是逐赛事端到端认证。页数、语言、模板与提交要求均以用户提供的当届官方材料为准。英文模板自动生成、非 CUMCM 最终门禁和 DOCX 导出尚未实现；详见 [比赛适配与工具边界](.agents/skills/cumcm-workflow/references/competition-adaptation.md)。
