...
**其他比赛可以复用建模、计算、复核方法和写作指导，但目前尚未全部兼容自动论文生成与最终交付。** 请按下面的支持层级使用：

| 比赛或场景 | 当前支持 | 尚需适配的部分 |
|---|---|---|
| CUMCM 国赛 | 现有建模、计算、复核、LaTeX/PDF 生成与交付链 | 仍须核对当届官方模板和提交要求 |
| 华为杯、MathorCup、电工杯及区域建模赛等固定题比赛 | 题意拆解、候选模型、计算记录与结果索引、机制验证、论文论证指导 | 对应比赛的论文模板与自动最终交付 |
| MCM/ICM、APMCM 等英文任务 | 上述共同方法，以及英文摘要、术语、按题目要求撰写的建议书等指导 | 英文模板自动生成与对应比赛的自动最终交付 |
| 泰迪杯及其他数据分析任务 | 数据口径、清洗、泄漏防范、基线与任务匹配的评价指导，可复用计算证据工具 | 按当届要求适配结果文件、论文及提交包 |
| 统计建模及自主选题任务 | 主题约束、数据可得性、研究设计、团队研究问题与证据范围指导 | 自选题项目的完整端到端验证及对应提交格式 |

**工具边界：**当前论文初始化器与模板 schema 仍绑定 CUMCM，自动交付仍要求 LaTeX/PDF；没有 DOCX 导出链。其他比赛可以先开展适用的分析、建模、计算和复核，并在结论确认后准备论文草稿，但不能把这些工作标记为已通过完整自动交付。

使用其他比赛时，向 agent 提供**比赛名称、年份、官方材料、论文语言和目标交付格式**，让它先读取[比赛适配指南](.agents/skills/cumcm-workflow/references/competition-adaptation.md)，说明当前可执行范围再继续。页数、语言、模板与提交要求均以当届官方材料为准。

上述列表表示指导覆盖与工具复用范围，并非逐赛事端到端测试认证；现有回归测试通过也不等于所有比赛已完成实题验收。
