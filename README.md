...
一套以全国大学生数学建模竞赛为基础的 AI agent 工作流：从官方材料出发，把建模、计算、复核、论文和最终交付串成一条可检查、可追溯的证据链。Skill 同时提供其他建模比赛的题型指导、自主选题和中英文写作方法。
...
**支持多比赛共用建模、计算、复核和中英文 LaTeX/PDF 交付链。** 论文初始化可指定真实比赛名称与语言，随后使用同一套编译、逐页检查、源码打包和人工确认流程。

| 比赛或场景 | 当前支持 | 使用时仍需核对 |
|---|---|---|
| CUMCM 国赛 | 原有完整流程；默认中文 CTeX 骨架 | 当届官方模板和提交要求 |
| 华为杯、MathorCup、电工杯及区域建模赛等固定题比赛 | 通用建模与验证指导、中文论文骨架、PDF 与源码交付 | 比赛专用排版、附件和结果文件要求 |
| MCM/ICM、APMCM 等英文任务 | 共用计算与复核链、英文论文骨架、英文写作指导、PDF 与源码交付 | 当届摘要页、页数统计、附加文书等要求 |
| 泰迪杯及其他数据分析任务 | 数据处理与评价指导，共用中英文论文及交付工具 | 指定结果表、数据字段和提交包内容 |
| 统计建模及自主选题任务 | 选题、数据可得性与研究设计指导，共用论文及交付工具 | 官方主题、实际研究设计和数据可支持的结论 |

**通用骨架不等于官方模板。** 初始化器不会根据比赛名称猜测页数或排版规则；存在已声明的官方论文模板时，仍须先采用或适配该模板。格式合规、实际页面质量和交付批准保留原有核验要求。DOCX 导出与任意官方模板的自动转换尚未提供。

使用时提供**比赛名称、年份、官方材料、论文语言和目标交付格式**。agent 在结论经你确认后，可按以下方式初始化论文，再接原有编译和交付流程：

```bash
python3 "$S/init_latex_paper.py" --project <project> --competition 'MCM/ICM' \
  --language en --competition-year <year> --title '<实际题名>' --keywords '<实际关键词>'
```

`S` 为 Skill 的 `scripts` 绝对路径。中文论文用 `--language zh`；省略新参数时仍按 CUMCM/中文初始化，原有调用与 v0.6 工作区无需迁移。详情见[比赛适配指南](.agents/skills/cumcm-workflow/references/competition-adaptation.md)。

验证覆盖中英文骨架的实际编译、渲染和打包，以及其他比赛的合成 finalizing 流程和原有关卡；这不等于逐赛事、逐年份的官方格式认证或完整赛题求解验收。
...
| **赛题原题或官方选题主题** | 必需 | PDF、Word 或其他官方版本，是题意、主题与约束的来源；团队自拟问题不能冒充官方题目 |
...
- **macOS / MacBook**：路径示例为 `/Users/你的名字/Documents/赛题资料`。
- **Windows（推荐 WSL2）**：在 WSL2 的 Linux 环境中运行 agent 与工作流工具。Windows 的 `C:\Users\你的名字\Documents\赛题资料` 在 WSL 中通常对应 `/mnt/c/Users/你的名字/Documents/赛题资料`；新项目路径可用 `/home/你的WSL用户名/cumcm-projects/2026B`。
- **Windows 原生 PowerShell**：后文的 Bash 示例，尤其 `$S`、`$PWD`、`ln -s`，不能直接照抄。当前 CI 在 Linux 上运行，尚未完成 Windows 原生端到端验证，因此优先使用 WSL2。

提示词中的材料路径和输出路径必须采用 **agent 实际运行环境**看到的格式，不要混用 Windows 与 WSL 路径。Python、所选计算后端和 XeLaTeX 也必须安装在该执行环境中。
