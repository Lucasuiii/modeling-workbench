...
- **默认充分论证、图文并重**：展开关键推导与结果解释，主动绘制有证据支持的图表和模型示意图，按比赛要求控制篇幅。
...
**如果 agent 下载仓库遇到网络问题，可以改用下面的 ZIP 补充方式完成第一步；原来的启动方式仍然可用。**

<details>
<summary>补充方式：提前下载 ZIP，从本地准备环境</summary>

先在浏览器中打开[仓库主页](https://github.com/Lucasuiii/modeling-workbench)，选择 **Code → Download ZIP**，完整解压后再让 agent 操作。已有完整本地副本可直接使用，无需重新下载。

将解压后的工作流放在独立工具目录，例如 `数学建模/modeling-workbench-main/`；官方材料另放一个目录，第二步的项目输出目录使用尚不存在的新目录。提供包含 `README.md` 和 `.agents/` 的那一层路径，不要提供 ZIP 文件或它的上级目录。`.agents` 是隐藏目录，解压时也需要保留。

在环境准备对话中，用下面的提示词替代第一步原提示词：

```text
请使用已下载并完整解压的本地 Modeling Workbench 准备环境。
工作流工具目录：/绝对路径/modeling-workbench-main

不要联网克隆仓库或检查更新，也不要尝试其他下载地址。
完整读取工具目录中的 .agents/skills/cumcm-workflow/SKILL.md，
先运行 scripts/doctor.py，报告可用阶段和缺失依赖。
若本地文件不完整，说明缺失项并停止获取仓库的尝试。
已有依赖不要重复安装；需要安装或修改环境时先列出变更并等我同意。
本次只准备环境，不初始化赛题；完成后报告实际工具目录、版本和缺失项。
```

随后继续原来的**第二步**，将工具目录替换为实际解压路径。ZIP 下载仍需要浏览器能访问 GitHub；这条路径避免 agent 反复尝试获取仓库，不代表依赖安装也能离线完成。缺少 LaTeX 等论文工具时，可以先开展基础环境支持的读题与建模，进入论文阶段前再补齐。
...
华为杯中文排版模板会在初始化识别到比赛名称时自动启用（可用 `--template generic` 改回通用模板），预设标题、正文、摘要和题注字号，支持按当届要求调整；另有[交付适配指南](.agents/skills/cumcm-workflow/references/huawei-delivery.md)：可装入按当届官方模板填写的封面 PDF，检查摘要页数、正文身份信息、提交文件名及附件，并记录最终 PDF 的 MD5。规则由项目提供，版式仍需逐页核对；这不代表已完成华为杯真题验收。
